...
## Experimental PoC 3.3: Blender Cloth physics

PoC 3.3 adds a self-contained Blender fallback for testing the final data
shape: a fixed semantic proxy skeleton drives collision geometry, Blender
Cloth generates the teacher cache, and the existing clean helper rig is shared
across ten motion sequences. Sequence H is a complete unseen physics-motion
holdout. The protocol, motion list, commands, leakage boundary, and visual
artifact steps are documented in
[docs/blender_physics_poc.md](docs/blender_physics_poc.md).

This experiment confirmed non-rigid cloth deformation and a measurable
history-dependent cloth state. The research model improved the primary H
mean-RMS over the rest baseline on the published local fixture; the
runtime-friendly candidate is the bounded 128-center RBF because its FBX
round-trip stayed stable; raw nearest-pose transforms were not accepted as a
game-ready export. Treat the result as an experimental cross-motion baseline,
not arbitrary humanoid inference or a finished VRChat runtime rig. The raw
physics teacher is generated by Blender's Cloth modifier and is never
procedurally replaced by a skeleton-only teacher.
